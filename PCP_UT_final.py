...
def main():
    if not is_process_running('PCP_2.0.6.exe'):
        current_directory = os.getcwd()
        exe_path = fr"{current_directory}/PCP/PCP_2.0.6.exe"
        print(exe_path)

        # Check if 'PCP' is in the list of folders
        process = multiprocessing.Process(target=run_exe, args=(exe_path,))
        process.start()
    else:
        print('PCP is already running')
...
class DesktopUtilityApp(QMainWindow):
    def __init__(self):
        ...
        exit_action.trig(gered.connect(self.close_application)
        menubar.addActionexit_action)
    ...
    def close_application(self):
        ...
        exe_path2 = fr"{current_directory}/PCP/kill_process.exe"
        print(exe_path2)
        process = multiprocessing.Process(target=run_exe, args=(exe_path2,))
        process.start()
...
if __name__ == "__main__":
    ...
    main()  # Call main to avoid the loop in subprocess
